...
-- PC-style server desktop + remote input gateway.
...
local function sendInput(d,data) if d and d.address and online(d) then modem.send(d.address,PORT,packet("INPUT",data)); return true end return false end
local function announce() broadcast("SERVER",{name="BULDACITY SERVER",role="SERVER",app="Buldacity OS",screen="SERVER ONLINE",remoteInput=true}) end
...
local function remotePage() titlebar(); card(3,5,74,H-9,"REMOTE CONTROL",C.pink); local d=listDevices()[selected]; if not d then text(7,9,"Select a device in DEVICE MANAGER first.",C.dim); taskbar(); return end; text(7,7,"DEVICE",C.dim); text(19,7,d.name or "UNKNOWN",C.cyan); text(7,9,"ROLE",C.dim); text(19,9,d.role or "CLIENT",C.white); text(7,11,"STATUS",C.dim); text(19,11,online(d) and "ONLINE" or "OFFLINE",online(d) and C.green or C.red); text(7,13,"APPLICATION",C.dim); text(19,13,d.app or "-",C.white); text(7,15,"ADDRESS",C.dim); text(19,15,d.address or "-",C.dim); text(7,18,"REMOTE INPUT",C.yellow); text(7,20,"Keyboard: forwarded to selected Tier-2 controller",C.white); text(7,21,"Touch: forwarded as remote touch coordinates",C.white); text(7,23,"Display mirroring: not available from the OC GPU API",C.dim); taskbar() end
...
while running do
  ...
  if e=="key_down" then
    local char=b or 0
    local key=c or 0
    local remote=listDevices()[selected]
    if page=="REMOTE" and remote and key~=17 and key~=19 then
      sendInput(remote,{event="key_down",char=char,code=key})
    elseif key==17 then running=false
    elseif key==2 then page="DESKTOP"
    elseif key==3 then page="DEVICES"
    elseif key==4 then page="REMOTE"
    elseif key==200 then selected=math.max(1,selected-1); page="DEVICES"
    elseif key==208 then selected=selected+1; page="DEVICES"
    elseif key==19 then announce() end
  elseif e=="key_up" then
    local remote=listDevices()[selected]
    if page=="REMOTE" and remote then sendInput(remote,{event="key_up",char=b or 0,code=c or 0}) end
  elseif e=="touch" then
    local x,y=a,b
    local list=listDevices()
    if y>=H-2 and x<27 then page="DESKTOP"
    elseif y>=H-2 and x<40 then page="DEVICES"
    elseif y>=H-2 and x<55 then page="REMOTE"
    elseif y>=H-2 and x<69 then announce()
    elseif page=="DEVICES" and y>=8 and y<=H-4 then selected=math.max(1,math.min(#list,y-7)); page="REMOTE"
    elseif page=="REMOTE" then
      local remote=list[selected]
      if remote then sendInput(remote,{event="touch",x=math.max(1,x-3),y=math.max(1,y-5),button=1}) end
    end
  elseif e=="scroll" and page=="REMOTE" then
    local remote=listDevices()[selected]
    if remote then sendInput(remote,{event="scroll",x=a or 0,y=b or 0,button=c or 0}) end
  elseif e=="modem_message" and c==PORT then
    local p=d
    if type(p)=="table" and p.protocol==PROTOCOL and p.kind~="SERVER" then upsert(b,p.data or {}) end
  end
