...
local C={bg=0x050811,panel=0x0B1220,panel3=0x182A40,line=0x24415C,cyan=0x20E6FF,blue=0x4D7CFF,green=0x37F59A,yellow=0xFFD35A,red=0xFF4D69,purple=0xB96CFF,white=0xEAF7FF,muted=0x71879E,dark=0x1B293A,black=0x000000}
...
local function scan()
 gates={}
 for address in component.list("stargate",true) do local p=component.proxy(address);if p then gates[#gates+1]={address=address,proxy=p} end end
 if selected>#gates then selected=#gates end;if selected<1 then selected=1 end
 if #gates==0 then logAdd("SCAN: NO SGCraft INTERFACE FOUND") else logAdd("SCAN: "..#gates.." STARGATE INTERFACE(S)") end
end
local function gate() return gates[selected] end
local function read(g)
 if not g then return nil end
 local p=g.proxy
 local state,engaged,direction=safe(p.stargateState);local la=safe(p.localAddress);local ra=safe(p.remoteAddress);local energy=safe(p.energyAvailable);local iris=safe(p.irisState);local need=nil
...
local function dial() if target=="" then logAdd("DIAL: ENTER 7 OR 9 SYMBOLS");return end;if #target~=7 and #target~=9 then logAdd("DIAL: ADDRESS NEEDS 7 OR 9 SYMBOLS");return end;if callGate("dial",target) then logAdd("DIAL STARTED -> "..target) end end
local function disconnect() if callGate("disconnect") then logAdd("GATE OFF / WORMHOLE DISCONNECTED") end end
...
local function layout()
 local compact=W<100 or H<30;local top=6;local logH=compact and 3 or 4;local bottomH=compact and 4 or (page=="DIAL" and 9 or 5);local contentH=H-top-logH-bottomH-2;if contentH<8 then contentH=8 end
...
local function drawHeader() fill(1,1,W,4,C.panel);text(2,1,"BULDACITY",C.cyan,C.panel);if W>=55 then text(14,1,"// SGCraft INTERFACE",C.white,C.panel) end;if W>=90 then text(W-20,1,"SGCRAFT / OC",C.muted,C.panel) end;text(2,2,"STARGATE CONTROL",C.green,C.panel);text(21,2,fit(notice,math.max(1,W-22)),C.muted,C.panel);fill(1,4,W,1,C.cyan) end
local function drawFleet(L)
 local x,y,w,h=2,L.top,L.leftW,L.contentH;panel(x,y,w,h,"GATES / INTERFACES",C.blue);if #gates==0 then text(x+2,y+3,fit("NO INTERFACE",w-4),C.red,C.panel);return end
...
local function drawDialPad(L,y)
 panel(2,y,W-3,L.bottomH,"DIAL ADDRESS - TOUCH KEYPAD",C.purple);text(4,y+1,"TARGET",C.muted,C.panel);text(13,y+1,fit(target=="" and "PRESS SYMBOLS" or target,W-32),C.cyan,C.panel)
 local chars="0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";local bw=math.max(4,math.floor((W-8)/12));local startY=y+3
...
local function drawBottom(L,gd)
 ...
 if L.compact then if page=="DIAL" then button("dial",W-13,y+1,11,"DIAL",C.green,false);button("clear",W-23,y+1,9,"CLR",C.red,false);text(4,y+2,fit(target=="" and "USE KEYBOARD" or target,W-28),C.cyan,C.panel) else text(4,y+2,"TOUCH NAV / GATE OFF / SCAN",C.cyan,C.panel) end
...
local function keyAction(code)
 ...
 if code==15 then page=page=="DIAL" and "HOME" or "DIAL";dirty=true;return end
...
local function click(id)
 ...
 if id:sub(1,5)=="gate_" then selected=tonumber(id:sub(6)) or selected;page="HOME";dirty=true;logAdd("GATE "..selected.." SELECTED");return end
...
scan();logAdd("TOUCH CONTROL READY - TOUCH A BUTTON")
while running do
 ...
 local ev,addr,a,b,c=event.pull(.10)
 if ev=="key_down" then
  local char=a;local keycode=b
  keyAction(keycode)
  if page=="DIAL" and char and char>=32 and char<=126 then local s=string.char(char):upper();if s:match("[0-9A-Z]") and #target<9 then target=target..s;dirty=true end end
 elseif ev=="touch" then
  local id=hit(a,b)
  if id then click(id) else notice="TOUCH "..tostring(a)..","..tostring(b);dirty=true end
