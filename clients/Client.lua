...
function M.start(name,controller,capabilities,extra,commandHandler)
  ...
  local ok,err=Network.startClient(M.name,info,function(packet)
    M.onPacket(packet)
  end)
...
function M.onPacket(packet)
  ...
  if packet.type=="COMMAND" then
    ...
    local result=false,"NO_COMMAND_HANDLER"
    if M.commandHandler then
      local ok,a,b=pcall(M.commandHandler,packet.payload or {},packet)
      if ok then result=a,b else result=false,a end
    end
    if result==nil then result=true end
    Network.send("STATUS", "TIER3-CORE", {
      role="CLIENT",
      nodeId=Network.status().nodeId,
      commandId=packet.id,
      success=result==true,
      result=b
    })
...
function M.sendCommandResult(commandId,success,result)
  return Network.send("STATUS","TIER3-CORE",{
    role="CLIENT",nodeId=Network.status().nodeId,
    commandId=commandId,success=success==true,result=result
  })
end
