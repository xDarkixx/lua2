...
if not component.isAvailable("internet") then
  error("BULDACITY RemoteBridge: Internet Card fehlt")
end
if TOKEN=="" then
  error("BULDACITY RemoteBridge: BULDACITY_GATEWAY_TOKEN fehlt")
end
...
local function get(url)
  ...
  local data=""
  local rok,r=pcall(function()for chunk in handle do data=data..chunk end end)
...
local function poll()
  local url=GATEWAY.."/bridge/poll?token="..TOKEN
...
event.timer(POLL_SECONDS,function()
  local ok,err=poll()
  if not ok then io.stderr:write("RemoteBridge: "..tostring(err).."\n") end
end,math.huge)
