...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path=="/":
            page=r'''<!doctype html><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><title>MODERN TIER-3</title><style>body{font-family:Inter,system-ui;background:#070b12;color:#e8f0ff;margin:0}main{max-width:1250px;margin:auto;padding:24px}.top{display:flex;justify-content:space-between;align-items:center}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:14px}.card{background:#0e1624;border:1px solid #26364d;border-radius:16px;padding:18px;box-shadow:0 10px 30px #0006}.muted{color:#7f91aa}.ok{color:#39ff9a}.pill{display:inline-block;padding:4px 8px;border-radius:999px;background:#15263a;margin:3px;font-size:12px}.app{padding:10px;border:1px solid #24344a;border-radius:10px;margin:6px 0}button,input{padding:9px;border-radius:9px;border:1px solid #30445e;background:#0a111d;color:#fff}button{cursor:pointer}.screen{font-family:monospace;line-height:1.1;background:#000;padding:10px;overflow:auto;white-space:pre}.screen span{display:inline-block;width:1ch;height:1em}</style><main><div class=top><div><h1>MODERN CONTROL CENTER</h1><div class=muted>Remote Modern UI • Tier-3 central network</div></div><button onclick=load()>Refresh</button></div><div id=cards class=grid><div class=card>Loading…</div></div><div class=card><h2>Remote command</h2><input id=d placeholder=Destination><input id=a placeholder=Action><input id=v placeholder=Value><button onclick=send()>Send</button><pre id=o></pre></div></main><script>const token=new URLSearchParams(location.search).get('token')||prompt('Modern Tier-3 Token');const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));function screen(x){let s=x.screen;if(!s||!s.rows)return '';let map={};s.rows.forEach(r=>map[r.y]=r.cells||[]);let out='';for(let y=1;y<=s.height;y++){let row=map[y]||[];row.forEach(c=>{let ch=String(c[0]||' ');if(ch===' ')ch='&nbsp;';out+=`<span title="${esc(c[0]||' ')}">${esc(ch)}</span>`});out+='\n'}return `<h3>Remote screen</h3><div class=screen>${out}</div>`}async function load(){let r=await fetch('/api/ui?token='+encodeURIComponent(token));let j=await r.json();let all=Object.values(j.ui||{});cards.innerHTML=all.length?all.map(x=>`<div class=card><h2>${esc(x.title||x.nodeId||'MODERN NODE')}</h2><div class=ok>● ONLINE</div><p class=muted>${esc(x.note||'Remote UI scene')}</p><button onclick="requestScreen('${esc(x.nodeId)}')">Refresh screen</button>${screen(x)}<h3>Modern applications</h3>${(x.apps||[]).map(a=>`<div class=app>${esc(a)}</div>`).join('')||'<span class=muted>none reported</span>'}<h3>Components</h3>${(x.components||[]).slice(0,12).map(c=>`<span class=pill>${esc(c.type)}</span>`).join('')||'<span class=muted>none reported</span>'}</div>`).join(''):'<div class=card>No Modern UI frames received yet.</div>'}async function requestScreen(node){await fetch('/api/command?token='+encodeURIComponent(token),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({destination:node,action:'SCREEN_REQUEST'})});setTimeout(load,1000)}async function send(){let r=await fetch('/api/command?token='+encodeURIComponent(token),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({destination:d.value,action:a.value,value:v.value})});o.textContent=await r.text()}load();setInterval(load,3000)</script>'''
