...
local function centralPacket(p)
  return p.route and p.route.central==true and p.route.via==TIER3_ID
end
...
local function forwardToTier3(sourceInterface,p)
  ...
  if not target then target=modems[sourceInterface] end
  local out=Protocol.forward(p)
  if not out or out.hops>=out.maxHops then return false,"TTL" end
  local raw=serialization.serialize(out)
  if #raw>Protocol.MAX_PACKET_BYTES then return false,"PACKET_TOO_LARGE" end
  local ok=send(target,tier3.address,raw,false)
  if ok then stats.forwarded=stats.forwarded+1 end
  return ok
end

local function forwardCentralPacket(sourceInterface,p)
  -- Only Tier-3-originated packets may cross from one segment to another.
  ...
  local out=Protocol.forward(p)
  if not out or out.hops>=out.maxHops then stats.dropped=stats.dropped+1;return false end
  local raw=serialization.serialize(out)
  if #raw>Protocol.MAX_PACKET_BYTES then stats.dropped=stats.dropped+1;return false end
  ...
  local ok=send(target,nil,raw,true)
  if ok then stats.forwarded=stats.forwarded+1 end
  return ok
end
...
event.listen("modem_message",function(_,receiver,sender,port,distance,raw)
  ...
  if p.source==TIER3_ID or centralPacket(p) then
    ...
    forwardCentralPacket(sourceInterface,p)
    return
  end

  -- Register local clients/relays at Tier-3. Never leak client packets to the
  -- opposite segment. This is the centralization rule.
...
local function announce()
  local p=Protocol.new("HELLO","RELAY-"..tostring(modems[1].address),"*",string.format("RELAY-%d",math.floor(now()*1000)),{
    role="RELAY",relay=true,interfaces=#modems,nodeId="RELAY",tier3=TIER3_ID
  })
