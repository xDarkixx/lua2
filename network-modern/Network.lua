...
-- Isolated from the legacy Network.lua on purpose.
...
local M={nodeId=nil,sequence=0,seen={},pending={},clientListening=false,serverListening=false}
...
function M.init(nodeId,port)
  if nodeId and tostring(nodeId)~="" then
    M.nodeId=tostring(nodeId)
  else
    local ok,a=pcall(computer.address)
    M.nodeId=ok and a or "NODE"
  end
...
function M.send(kind,destination,payload,address)
  ...
  return sendPacket(packet,address,address==nil)
end
...
function M.sendReliable(kind,destination,payload,address)
  ...
  local raw=serialization.serialize(packet)
  if #raw>Protocol.MAX_PACKET_BYTES then return false,"PACKET_TOO_LARGE" end
  M.pending[packet.id]={packet=packet,raw=raw,address=address,retries=0,deadline=now()+ACK_TIMEOUT}
  local ok,err
  if address then ok,err=Transport.send(address,raw) else ok,err=Transport.broadcast(raw) end
  if not ok then M.pending[packet.id]=nil;return false,err end
  return true,packet.id
end

function M.hello()
  return M.send("HELLO","*",{role="NODE",node=M.nodeId})
end

function M.heartbeat()
  return M.send("HEARTBEAT","*",{role="NODE",node=M.nodeId})
end

local function handleAck(p)
  local d=p.payload or{}
  local ackId=d.ackId or d.id
...
local function ack(r,p)
  ...
  local destination=p.source
  local response=Protocol.new("ACK",M.nodeId,destination,nextId(),{ackId=p.id,ackType=p.type})
...
function M.poll(timeout)
  ...
  if not ok then return nil,"INVALID_PACKET" end
  local valid,reason=Protocol.valid(p)
  if not valid then return nil,"INVALID_PACKET_"..tostring(reason) end
...
function M.startClient(name,extra,callback)
  ...
  M.clientExtra=extra or{}
  M.clientExtra.name=M.clientName
  M.clientExtra.role="CLIENT"
  M.clientExtra.nodeId=M.nodeId
  M.clientExtra.protocol=Protocol.NAME
  M.clientExtra.port=Protocol.PORT
  if not M.clientListening then
    ...
    event.listen("modem_message",function(_,receiver,sender,port,distance,raw)
      ...
      if p.type=="ACK" or p.type=="NACK" then handleAck(p);return end
      if p.type=="HELLO" then
        M.serverAddress=sender
        M.send("ACK",p.source,{ackId=p.id,ackType="HELLO",server=true},sender)
      elseif p.type=="STATUS" and type(p.payload)=="table" then
        M.serverAddress=sender
  ...
  if not M.heartbeatTimer then
    M.heartbeatTimer=event.timer(3,function()M.heartbeat();M.retryPending() end,math.huge)
  end
...
function M.startServer(callback)
  ...
  event.listen("modem_message",function(_,receiver,sender,port,distance,raw)
    ...
    Registry.touch(p.source,sender,p.payload and p.payload.kind)
    if p.type=="HELLO" then
      M.send("STATUS",p.source,{role="SERVER",nodeId=M.nodeId,protocol=Protocol.NAME,port=Protocol.PORT},sender)
      M.send("ACK",p.source,{ackId=p.id,ackType="HELLO",server=true},sender)
    elseif p.type=="PING" then
      M.send("PONG",p.source,{id=p.id,server=true,nodeId=M.nodeId},sender)
...
function M.status()
  ...
  return {ok=ok,transport=info,nodeId=M.nodeId,protocol=Protocol.NAME,port=Protocol.PORT,pending=M.pending}
end
