  ...
  for y in range(s.height):
   for z in range(s.length):
    for x in range(s.width):
     ...
     if b or include_air:f.write(f'{x},{y},{z}={b}:{m}\n')
def import_txt(path):
 name='Schematic';materials='Alpha';size=None;blocks={}
 ...
 for no,line in enumerate(lines[1:],2):
  ...
  if s.startswith('size='):
   ...
   if len(size)!=3:raise ValueError(f'Zeile {no}: size muss W,H,L sein.')
   continue
  m=RX.match(s)
  ...
  if bid>4095 or meta>15:raise ValueError(f'Zeile {no}: ungültige Block-ID/Metadata.')
  blocks[(x,y,z)]=(bid,meta)
