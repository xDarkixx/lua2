-- BULDACITY Network.lua
-- Single public Network API. Legacy BULDACITY/2 transport is removed.
-- All traffic is handled by network-modern and TIER3-CORE.
...
function M.valid(p)if type(p)~="table"then return false end;if p.protocol==M.PROTOCOL and type(p.kind)=="string"then return true end;local ok=Protocol.valid(p);return ok==true end

-- Compatibility signature: Network.send(address,kind,data).
-- It uses the modern protocol/port and central routing infrastructure.
...
function M.startClient(name,extra,callback)local function cb(p,sender,distance)M.serverAddress=Modern.serverAddress or M.serverAddress;M.lastPacketDistance=tonumber(distance)or 0;local q=legacy(p);if q and q.sender then local d=M.diagnostics[q.sender]or{address=q.sender};d.last=computer.uptime();d.distance=M.lastPacketDistance;d.result="ONLINE";M.diagnostics[q.sender]=d end;if callback then pcall(callback,q,sender,distance)end end;local ok,err=Modern.startClient(name or "BULDACITY CONTROLLER",extra or{},cb);return ok,err end
function M.startServer(callback)local function cb(p,sender,distance)M.lastPacketDistance=tonumber(distance)or 0;local q=legacy(p);local source=q and q.sender or sender;if source then local d=M.diagnostics[source]or{address=source};d.last=computer.uptime();d.distance=M.lastPacketDistance;d.result="ONLINE";if q.data then for k,v in pairs(q.data)do d[k]=v end end;M.diagnostics[source]=d end;if callback then pcall(callback,q,sender,distance)end end;local ok,err=Modern.startServer(cb);M.serverAddress=Modern.serverAddress;return ok,err end
