...
-- Adaptive neon interface with live crafting, P2P/OC discovery and scrollable recipes.
...
-- P2P/OC discovery is deliberately capability-based. AE2 rv3 adds the OC P2P tunnel,
-- but the old integration does not guarantee a Lua component with configuration methods.
-- We therefore inspect exposed component types and methods instead of inventing an API.
...
local function methodNames(doc)
 local out={}
 if type(doc)=="table" then
  for name,_ in pairs(doc) do table.insert(out,tostring(name)) end
 end
 table.sort(out)
 return out
end
...
local function scanP2P()
 ...
 if type(iter)=="function" then
  for addr,kind in iter do
   ...
   if score>0 then
    local methods=methodNames(doc)
    table.insert(p2ps,{address=tostring(addr),kind=tostring(kind),score=score,methods=methods})
   end
  end
 end
...
local function header(title)
 ...
 led(math.max(5,W-27),2,true,blink and C.green or C.cyan,"ONLINE")
 line(1,4,W,C.cyan)
end
...
local function drawHome()
 ...
 txt(6,cy+14,"P2P CANDIDATES",C.grey,C.panel);txt(math.min(27,pw-8),cy+14,#p2ps,C.orange,C.panel)
 txt(6,cy+16,"SCAN AGE",C.grey,C.panel);txt(math.min(27,pw-8),cy+16,string.format("%.1fs",computer.uptime()-lastScan),C.white,C.panel)
 if ph>=20 then txt(6,cy+18,"NETWORK LOAD",C.grey,C.panel);bar(6,cy+19,math.max(5,pw-12),math.min(100,#items/1000*100),C.cyan) end
 if two then
  local x=3+pw+gap
  ...
  txt(x+4,cy+ph-4,"P2P MONITOR",C.grey,C.panel)
  marqueeBar(x+4,cy+ph-2,math.max(5,pw-8),pulse,C.orange)
 end
 footer()
end
...
local function drawCrafts()
 ...
 local jobsNow=activeJobCount()
 txt(x+3,y+16,"LIVE JOBS",C.grey,C.panel);txt(x+3,y+17,jobsNow,C.green,C.panel)
 marqueeBar(x+3,y+19,math.max(5,right-6),pulse,C.green)
 txt(x+3,y+h-3,"WHEEL / ▲▼ / ↑↓",C.grey,C.panel)
 footer()
end
...
local function drawJobs()
 ...
 local y=6;local h=math.max(4,H-10);local w=W-6
 panel(3,y,w,h,"CRAFTING STATUS",C.green)
 local yy=y+2
 if #jobs==0 then txt(7,yy,"NO LOCAL CRAFTING REQUESTS",C.grey,C.panel) end
 for i=1,math.min(#jobs,math.max(1,h-4)) do
  local job=jobs[i];local state,col,done=jobState(job)
  txt(6,yy,string.format("%02d",i),C.cyan,C.panel)
  txt(10,yy,fit(job.name,math.max(10,w-34)),C.white,C.panel)
  txt(math.max(1,W-20),yy,state,col,C.panel)
...
local function drawP2P()
 ...
 local y=6;local h=math.max(4,H-10);local w=W-6
 local left=math.max(38,math.floor(w*0.58));local right=w-left-2
 panel(3,y,left,h,"P2P / OC DISCOVERY",C.orange)
 txt(6,y+2,"EXPOSED CANDIDATES",C.grey,C.panel)
 txt(math.max(1,3+left-12),y+2,#p2ps,C.orange,C.panel)
 local yy=y+4
 if #p2ps==0 then
  ...
  for i=1,math.min(#p2ps,math.max(1,h-7)) do
   local p=p2ps[i]
   led(6,yy,true,C.orange,fit(p.kind,math.max(10,left-12)))
   txt(6,yy+1,fit(p.address,math.max(10,left-12)),C.grey,C.panel)
   txt(6,yy+2,"MATCH SCORE "..tostring(p.score),C.yellow,C.panel)
   if #p.methods>0 then txt(6,yy+3,fit(table.concat(p.methods,", "),math.max(10,left-12)),C.cyan,C.panel) else txt(6,yy+3,"NO DOCUMENTED METHODS",C.grey,C.panel) end
   yy=yy+5
   if yy>y+h-5 then break end
  end
 end
 local x=3+left+2
 panel(x,y,right,h,"P2P STATUS",C.purple)
 led(x+3,y+3,#p2ps>0,C.orange,#p2ps>0 and "OC P2P CANDIDATE" or "CONFIGURED IN AE2")
 txt(x+3,y+6,"LAST SCAN",C.grey,C.panel)
 txt(x+3,y+7,string.format("%.1fs",computer.uptime()-lastP2PScan),C.white,C.panel)
 txt(x+3,y+9,"CONTROL MODE",C.grey,C.panel)
 txt(x+3,y+10,#p2ps>0 and "CAPABILITY DISCOVERY" or "AE2 MEMORY CARD",C.orange,C.panel)
 button("p2pscan",x+3,y+13,math.max(8,right-6),"SCAN P2P",C.orange)
 txt(x+3,y+16,"NO FAKE API CALLS",C.green,C.panel)
 txt(x+3,y+18,"BULDACITY monitors what",C.grey,C.panel)
 txt(x+3,y+19,"the installed OC bridge exposes.",C.grey,C.panel)
 footer()
end
...
local function drawSystem()
 ...
 txt(6,y+9,"COLOR DEPTH",C.grey,C.panel);txt(26,y+9,tostring(depth or "?"),C.pink,C.panel)
 local total=computer.totalMemory and safe(computer.totalMemory) or 0
 local free=computer.freeMemory and safe(computer.freeMemory) or 0
...
local function draw()
 ...
 elseif page=="P2P" then drawP2P()
 elseif page=="SYSTEM" then drawSystem()
 else page="HOME";drawHome() end
end
...
local function click(x,y)
 ...
 elseif hit("p2p",x,y) then page="P2P"
 ...
 elseif page=="CRAFTS" then
  for i=1,#crafts do if hit("craftrow"..i,x,y) then selectCraft(i);break end end
...
while running do
 ...
 if e=="touch" then
  -- touch(screenAddress, x, y, button, playerName)
  click(b,c)
  draw()
 elseif e=="scroll" then
  -- scroll(screenAddress, x, y, direction, playerName); direction is argument c.
  if page=="CRAFTS" then
 ...
 elseif e=="key_down" then
  -- key_down(keyboardAddress, char, code, playerName)
  local code=tonumber(c) or 0
  if code==200 and page=="CRAFTS" then scrollCraft(-1);draw()
  elseif code==208 and page=="CRAFTS" then scrollCraft(1);draw()
  elseif code==203 and page=="CRAFTS" then selectCraft(selectedCraft-1);draw()
  elseif code==205 and page=="CRAFTS" then selectCraft(selectedCraft+1);draw()
  elseif code==28 and page=="CRAFTS" then requestCraft(selectedCraft,1);draw()
  elseif code==1 then running=false
  end
...
clear(C.bg)
txt(3,3,"BULDACITY AE2 CORE OFFLINE",C.cyan,C.bg)
txt(3,5,"OpenComputers session ended.",C.grey,C.bg)
