...
if HEIGHT < HOLOW+2 then
  error("[BŁĄD] Twój monitor lub/i karta graficzna nie obsługują wymaganej rozdzielczości")
else
  WIDTH = HOLOW*2+40
  HEIGHT = HOLOW+2
...
-- draw "brush selector" menu
function drawColorSelector()
  frame(MENUX, 3, WIDTH-2, 16, "[ Paleta ]")
  for i=0, 3 do
...
function drawLayerSelector()
  frame(MENUX, 16, WIDTH-2, 28, "[ Warstwa ]")
  gpu.set(MENUX+13, 18, "Wartsta hologramu:")
  gpu.set(MENUX+1, 23, "Warstwa 'duch':")
end
function drawButtonsPanel()
  frame(MENUX, 28, WIDTH-2, 36, "[ Zarządzanie ]")
end
...
function saveHologram()
  ...
  if filename ~= FILE_REQUEST then
    -- warning
    showMessage('Zapisywanie... Proszę czekać.', '[ Uwaga ]', goldcolor)
    -- add brand extension =)
    ...
    save(filename)
    -- done info
    showMessage('   Plik został zapisany!  ', '[ Gotowe ]', goldcolor)
    repaint = true
  end
end
...
function loadHologram()
  ...
  if filename ~= FILE_REQUEST then
    -- warning
    showMessage('Ładowanie...', '[ Uwaga ]', goldcolor)
    -- add brand extension =)
    if string.sub(filename, -3) ~= '.3d' then
      filename = filename..'.3d'
    end
...
buttonsNew(exit, WIDTH-BUTTONW-2, HEIGHT-2, 'Wyjście', errorcolor, BUTTONW)
buttonsNew(drawLayer, MENUX+10, 14, 'Odśwież', goldcolor, BUTTONW)
buttonsNew(prevLayer, MENUX+1, 19, '-', infocolor, 5)
buttonsNew(nextLayer, MENUX+7, 19, '+', infocolor, 5)
buttonsNew(setTopView, MENUX+1, 21, 'Góra', infocolor, 10)
buttonsNew(setFrontView, MENUX+12, 21, 'Przód', infocolor, 10)
buttonsNew(setSideView, MENUX+24, 21, 'Strona', infocolor, 9)

buttonsNew(prevGhost, MENUX+1, 24, 'Poniżej', infocolor, 5)
buttonsNew(nextGhost, MENUX+11, 24, 'Powyżej', infocolor, 5)

buttonsNew(clearLayer, MENUX+1, 26, 'Wyczyść', infocolor, BUTTONW)
buttonsNew(fillLayer, MENUX+2+BUTTONW, 26, 'Wypełnij', infocolor, BUTTONW)

buttonsNew(drawHologram, MENUX+8, 30, 'Na projektor', goldcolor, 16)
buttonsNew(saveHologram, MENUX+1, 33, 'Zapisz', helpcolor, BUTTONW)
buttonsNew(loadHologram, MENUX+8+BUTTONW, 33, 'Ładuj', infocolor, BUTTONW)
buttonsNew(newHologram, MENUX+1, 35, 'Nowy plik', infocolor, BUTTONW)
...
tb_ghostlayer = textboxesNew(setGhostLayer, MENUX+21, 24, ' ', WIDTH-MENUX-23)
FILE_REQUEST = 'Podaj nazwę pliku'
