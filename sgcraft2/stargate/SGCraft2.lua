...
-- Designed around a graphical SGC / SG-1 inspired HUD: rings, glyphs, chevrons,
-- event horizon, iris, telemetry and persistent address storage.
...
local function logAdd(s)
  ...
  while #logs>18 do table.remove(logs,1) end
  notice=tostring(s)
end
...
local function saveBook()
  ...
  logAdd("ADDRESS BOOK / SAVED / "..#book.." RECORDS")
end
...
local function iris()
  local g=current();if not g then return end
  local d=API.read(g);local closed=string.lower(d.iris or "")=="closed"
  local ok,x,e
...
local function drawTelemetry(x,y,w,h,d)
  ...
  row(11,"IRIS",d.iris,d.iris=="Closed" and C.green or C.yellow)
...
local function drawBook()
  ...
  text(5,9,"SEARCH",C.muted,C.metal);fill(13,8,30,3,C.metal2);text(15,9,search=="" and "TYPE TO FILTER" or search,C.cyan,C.metal2)
...
local function handle(id)
  ...
  elseif id:sub(1,4)=="gate" then selected=tonumber(id:sub(5)) or selected;saveInterface()
  ...
  elseif id:sub(1,3)=="key" then
    local i=tonumber(id:sub(4));local chars="1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZ";target=normalize(target..chars:sub(i,i))
  end
end
...
while running do
  ...
  if now>=nextFrame then anim=anim+0.10;nextFrame=now+0.10;draw() end
  ...
  elseif e[1]=="sgStargateStateChange" or e[1]=="sgChevronEngaged" or e[1]=="sgIrisStateChange" then
    logAdd("EVENT / "..tostring(e[1]));draw()
