...
-- Tests every discovered client with PING/PONG and reports wired/wireless,
...
local ok,mode=network.startServer(function(sender,p,distance)
 ...
 elseif p.kind=="PONG" then
  ...
  if d.pingSent then d.latency=(now()-d.pingSent)*1000 end
 end
end)
...
local function discovery()
 ...
 network.broadcast("SERVER_HELLO",{name="BULDACITY NETWORK TEST",role="SERVER",app="NETWORK TEST",protocol=PROTOCOL,port=4242})
...
local function testAll()
 ...
 if #list==0 then log("FAIL: no controller clients discovered");return false end
 print("\n============================================================")
 print(" BULDACITY/2 END-TO-END NETWORK TEST")
 print("============================================================")
 local all=true
 for i,d in ipairs(list) do
  d.pong=false;d.pingSent=now();d.latency=nil
  local sent=network.send(d.address,"PING",{from="BULDACITY NETWORK TEST",id=tostring(d.pingSent)})
  if not sent then d.result="SEND FAIL";all=false
  else
  ...
  print(string.format("%02d %-28s %-5s %-8s dist=%-4s latency=%-7s relay=%s",i,tostring(d.name or d.address):sub(1,28),d.result,link,tostring(d.distance or 0),d.latency and string.format("%.1fms",d.latency) or "--",relay))
 end
 print("------------------------------------------------------------")
...
discovery();testAll()
print("Press R to run again, Q to quit.")
while running do
 local e,_,char=event.pull(0.25)
 if e=="key_down" then
  if char==114 or char==82 then discovery();testAll()
  elseif char==113 or char==81 then running=false end
