...
function UI.draw(d)
 ...
 if #d.gates==0 then txt(left+2,9,"NO INTERFACES",UI.C.red,UI.C.panel);txt(left+2,11,"SCAN REQUIRED",UI.C.muted,UI.C.panel) else for i,g in ipairs(d.gates) do local y=8+(i-1)*3;if y>UI.H-5 then break end;local on=i==d.selected;rect(left+1,y,lw-2,2,on and UI.C.panel2 or UI.C.panel);txt(left+3,y,(on and "▶ " or "  ")..i,UI.C.cyan,on and UI.C.panel2 or UI.C.panel);txt(left+8,y,fit(g.address,14),UI.C.white,on and UI.C.panel2 or UI.C.panel);led(left+8,y+1,g.state=="Connected",g.state=="Connected" and UI.C.green or UI.C.blue,g.state or "Offline") end end
 ...
 if not g then txt(right+3,9,"Select a Stargate Interface",UI.C.red,UI.C.panel);txt(right+3,11,"Use SCAN to discover gates.",UI.C.muted,UI.C.panel) else
  ...
  txt(right+3,12,"LOCAL",UI.C.muted,UI.C.panel);txt(right+3,13,fit(g.localAddress or "-",16),UI.C.white,UI.C.panel)
 ...
 if d.page=="dial" then
  panel(left,UI.H-12,UI.W-3,7,"DIAL TARGET",UI.C.purple);txt(left+3,UI.H-10,"TARGET",UI.C.muted,UI.C.panel);txt(left+12,UI.H-10,fit(d.target=="" and "TYPE ADDRESS" or d.target,UI.W-17),UI.C.cyan,UI.C.panel);txt(left+3,UI.H-8,"ENERGY",UI.C.muted,UI.C.panel);txt(left+12,UI.H-8,tostring(d.energyNeed or "-").." / "..tostring((g and g.energy) or 0).." SU",UI.C.yellow,UI.C.panel);UI.button("dialNow",left+3,UI.H-6,14,"DIAL",UI.C.green,false);UI.button("disconnect",left+19,UI.H-6,18,"DISCONNECT",UI.C.red,false)
 elseif d.page=="iris" then
  panel(left,UI.H-12,UI.W-3,7,"IRIS SECURITY",UI.C.yellow);txt(left+3,UI.H-10,"CURRENT",UI.C.muted,UI.C.panel);txt(left+13,UI.H-10,(g and g.iris) or "-",UI.C.yellow,UI.C.panel);UI.button("openIris",left+3,UI.H-6,14,"OPEN",UI.C.green,false);UI.button("closeIris",left+19,UI.H-6,14,"CLOSE",UI.C.red,false)
 elseif d.page=="link" then
  panel(left,UI.H-12,UI.W-3,7,"SECURE LINK",UI.C.green);txt(left+3,UI.H-10,"MESSAGE",UI.C.muted,UI.C.panel);txt(left+13,UI.H-10,fit(d.message=="" and "TYPE MESSAGE" or d.message,UI.W-17),UI.C.white,UI.C.panel);UI.button("sendMessage",left+3,UI.H-6,14,"SEND",UI.C.green,false);local yy=UI.H-3;for i=math.max(1,#d.log-2),#d.log do txt(left+3,yy,fit(d.log[i],UI.W-7),UI.C.muted,UI.C.panel);yy=yy+1 end
 end
 local y=UI.H-4;local labels={{"status","STATUS",UI.C.cyan},{"gates","GATES",UI.C.blue},{"dial","DIAL",UI.C.purple},{"iris","IRIS",UI.C.yellow},{"link","LINK",UI.C.green},{"scan","SCAN",UI.C.cyan}};local bw=math.max(8,math.floor((UI.W-3-#labels)/#labels));local x=2;for _,b in ipairs(labels) do UI.button(b[1],x,y,bw,b[2],b[3],d.page==b[1]);x=x+bw+1 end
 txt(2,UI.H-1,"TAB NEXT   D DIAL   X DISCONNECT   O/C IRIS   M SEND   Q EXIT",UI.C.muted,UI.C.bg)
end
return UI
